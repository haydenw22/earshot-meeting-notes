...
RELEASES: tuple[Release, ...] = (
    ...
    Release(
        ...
        (
            ("Fixed", (
                "Renewal, trial and beta end dates now include the year whenever "
                "it is not the current year. A subscription ending in a future "
                "year used to render as just \"June 26\", which read as a date "
    ...
    Release(
        ...
        (
            ("Added", (
                "Earshot now runs natively on Apple Silicon Macs (macOS 14.4 or "
                "newer). Download Earshot.dmg from the release and drag Earshot to "
                "Applications.",
                "Both sides of a meeting are captured on the Mac just like on "
                "Windows: your microphone plus system audio. System audio uses "
                "Apple's own audio tap, so there is no virtual driver to install "
                "and it does not capture the screen. macOS asks once for System "
                "Audio Recording permission and once for the microphone before "
                "the first recording begins.",
                "Automatic updates work on the Mac too, with the same protection as "
                "Windows plus native platform checks: every download is verified, "
                "and the replacement must be a newer notarized Earshot build signed "
                "by the same developer. The old app is retained until the new one "
                "has launched successfully.",
                "Approximate call detection on the Mac can offer to record when an "
                "input is active while Zoom, Teams, Webex, Slack or FaceTime is open. "
                "macOS does not reveal which app owns the microphone, so browsers are "
                "excluded and the limitation is disclosed in Settings.",
            )),
            ("Notes", (
    ...
    Release(
        ...
        (
            ("Added", (
                ...
                "computer and anyone with the link can view them.",
                "Re-sharing the same meeting updates the page in place and keeps the "
                "same link. A shared meeting shows its link in the Share dialog with "
                "Copy link, Update and Stop sharing; stopping takes the page down "
                "immediately.",
                "Action items now have Keep all and Delete all buttons in the section "
                "header while AI suggestions are waiting for review, so cleaning up "
            ...
            ("Notes", (
                ...
                "server release 2026-07-13 or later.",
            )),
        ),
    ),
    Release(
        ...
        (
            ("Added", (
                "Updates are now cryptographically verified before they run. Every "
                "release publishes a SHA-256 digest next to the installer, and the app "
                "refuses to install any download that does not come from GitHub over "
                "HTTPS, look like a real Windows installer and match that digest "
                "exactly.",
                "If Earshot can no longer save audio mid-recording (disk full, "
                "recordings folder disconnected, permissions changed), a red banner "
                "appears within moments and the problem is recorded on the meeting, "
                "instead of the level meters happily bouncing while audio is silently "
                "lost.",
            )),
            ("Fixed", (
                "Closing the update window during a download now cancels the download "
                "cleanly. Previously it kept running invisibly and could still launch "
                "the installer (closing the app under you) after you had dismissed it, "
                "or crash the app on exit.",
                "Deleting a meeting now only removes it from your library once its "
                "recording files are verifiably gone. If a file is locked or "
                "undeletable, the meeting is kept and Earshot tells you exactly which "
                "folder still holds audio, so sensitive recordings can no longer be "
                "silently left behind.",
                "Sending action items to Todoist no longer duplicates tasks when one "
                "item in the batch fails: tasks created before the failure are "
                "remembered, so a retry only sends what is missing.",
                "Ask Earshot citation timestamps are now read from the transcript "
                "line where the quote actually occurs instead of trusting the AI's "
                "stated time, and citations with empty quotes are rejected.",
            )),
            ("Changed", (
                "The \"after AI summary\" webhook now fires only when a summary "
                "actually exists, including after a successful Re-summarise, and "
                "never for a failed one.",
                "The temporary 16 kHz upload copies made for transcription are "
                "deleted once transcription finishes instead of being kept forever "
                "next to the originals, saving disk space on every meeting.",
                "The Earshot Plus endpoint setting only accepts HTTPS (or a local "
                "development server), so a corrupted config file cannot quietly "
                "redirect your account token somewhere else.",
                "Links inside AI action results are display-only and no longer "
                "clickable.",
            )),
        ),
    ),
    ...
    Release(
        ...
        (
            ("Fixed", (
                "A translucent ghost copy of the app could flash on screen for a "
                "split second when opening Overview or refreshing a page. Gone.",
            )),
        ),
    ),
    Release(
    ...
    Release(
        ...
        (
            ("Added", (
                "Ask Earshot now saves your conversations. A new History button next to New "
                "chat lists your past chats (auto-named from your first question) so you can "
                "reopen and continue them.",
                "You can select and copy answer text, and each answer has a Copy button that "
                "puts the whole response on your clipboard — handy for pasting into a team "
                "chat or email.",
            )),
            ("Changed", (
                "Ask Earshot will now draft the emails, messages and other follow-ups your "
                "meetings call for, instead of refusing because a task was \"assigned to "
                "you\". If details are missing it fills the gaps with [brackets] for you to "
                "complete. (Earshot Plus users get this once the server updates.)",
            )),
        ),
    ),
    Release(
    ...
    Release(
        ...
        (
            ("Fixed", (
                "The sidebar Help button did nothing in 0.29.1: an internal error "
                "stopped its menu from opening. It pops up reliably again, and a test "
                "now clicks the real button so this can't quietly break twice.",
            )),
        ),
    ),
    Release(
    ...
    Release(
        ...
        (
            ("Added", (
                ...
                "Bookmarks: flag key moments while recording with the button or the native B "
                "shortcut (Ctrl+B on Windows, Command+B on Mac), then jump "
