"""Lucide-style SVG icons rendered to themeable QIcons/QPixmaps.

Icons are stroke-based vectors (consistent 2px stroke, round caps) recoloured at
render time, so the same glyph works in light and dark themes and stays crisp on
high-DPI displays.
"""
...
def _svg(name: str, color: str, size: int) -> str:
    body = ICONS.get(name, "")
    if name in _FILLED:
...
def pixmap(name: str, color: str, size: int = 20) -> QPixmap:
    dpr = _dpr()
    renderer = QSvgRenderer(QByteArray(_svg(name, color, size).encode("utf-8")))
